...
data01=dplyr::select(data,glucose,age)
head(data01)

data02=data %>% dplyr::select(glucose,age) %>% head()
```
...
**all_of()**: Identique à tout (), sauf qu'aucune erreur n'est renvoyée pour les noms qui ne exister.
...
Useful mutate functions +, -, log(), etc., for their usual mathematical meanings lead(), lag() dense_ank(), min_ank(), percent_rank(), row_number(), cume_dist(), ntile() cumsum(), cummean(), cummin(), cummax(), cumany(), cumall() na if(), coalesce() if else(), recode(), case when()
